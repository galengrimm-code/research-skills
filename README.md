...
> Four complementary research skills for Claude Code: general internet research, security-focused exposure analysis, full corpus extraction, and append-only delta updates of existing archives. v3.4.0 — synthesis-quality upgrades inspired by the Search → Expand → Compare → Model inquiry loop: **evidence-chain tracing** in `/research` and `/deep-research` (trace load-bearing claims back to their origin; ten outlets echoing one press release count as ONE source, not ten — kills citation laundering), a required **Settled / Contested / Unknown** triage section in every `/research` report, **falsifiability sections** ("What would change this answer" in `/research` How-to and Comparison modes; "What would flip this verdict" in `/deep-research` exposure calls), and **Next-Pass Questions** at the end of every `/research` report (the recursive-loop output — each pass ends with sharper follow-up questions, rendered as ready-to-run invocations in the report and persisted as plain questions in `recipe.yaml`). v3.3.0 added a **Pulse / Recency** mode to `/research` (community sentiment over a recent window), pulled **Hacker News** into both `/research` and `/deep-research`, and routed Reddit via keyless `.rss` + `old.reddit.com` HTML. v3.2.0 added a shared YouTube transcript fallback chain (yt-dlp → youtube-transcript-api → Whisper); default Whisper model `large-v3-turbo`.
